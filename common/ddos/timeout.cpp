...
namespace engine {
	...
	namespace ddos {
		...
		void Timeout::new_msg(Message message){
			...
			if(message.getwhat() == S_WhatType::W_NOTI_RECV_CLOSED){
				if(0 < item.netid){
			...
			if(message.getwhat() == S_WhatType::W_NOTI_RECV_NORMAL){
		...
		void Timeout::new_msg(vector<Message> msgs){
		...
		void Timeout::check(Thread thread){
			...
			while(it != this->m_items.end()){
				...
				if(interval > item.timeout){
					Message message;
					message.setwhat(S_WhatType::W_NOTI_RECV_TIMEOUT);
		...
		void Timeout::on_noti_timeout(Message message){
			if(0 == message.getsize()){
				return ;
			}
		...
		void Timeout::noti_ddos_thread(Thread thread,const int netid,
		...
		void Timeout::noti_ddos_thread(const string name,const int netid,
			int timeout,S_WhatType type){
